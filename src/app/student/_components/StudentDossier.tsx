        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          ...
          <div className="flex items-center gap-2 shrink-0 flex-wrap sm:flex-nowrap w-full sm:w-auto">
            <button
              ...
              className="flex-1 sm:flex-none px-3.5 py-1.5 bg-white/15 hover:bg-white/25 text-white font-bold text-xs rounded-xl border border-white/20 transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-xs active:scale-95"
            >
            ...
            <button
              ...
              className="flex-1 sm:flex-none px-3.5 py-1.5 bg-emerald-800/80 hover:bg-emerald-700 text-emerald-100 font-bold text-xs rounded-xl border border-emerald-600/80 transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-xs active:scale-95"
            >
      ...
      <div className="bg-slate-100/90 rounded-3xl border border-slate-300 p-4 sm:p-5 space-y-3 shadow-2xs">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-xs">
          <div className="p-2.5 bg-slate-50 border border-slate-200/90 rounded-xl space-y-0.5 shadow-2xs">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Mobile Phone</span>
            <p className="font-mono font-bold text-slate-900">{student.mobile || 'Not set'}</p>
          </div>

          <div className="p-2.5 bg-slate-50 border border-slate-200/90 rounded-xl space-y-0.5 shadow-2xs">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Guardian Phone</span>
          ...
          <div className="p-2.5 bg-slate-50 border border-slate-200/90 rounded-xl space-y-0.5 sm:col-span-2 lg:col-span-3 shadow-2xs">
          ...
          <div className="space-y-1.5">
            {recentActivities.length > 0 ? (
              recentActivities.map((act) => (
                <div key={act.aid} className="p-2.5 bg-indigo-100/50 hover:bg-indigo-100/80 border border-indigo-200/90 rounded-xl flex items-center justify-between text-xs transition-colors shadow-2xs">
                  <div>
                    <span className="font-bold text-slate-900 text-[11px] block">{act.subjectTuitioned || 'Study Session'}</span>
                    <span className="text-[10px] text-slate-600 font-mono">{act.date}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span
                      className={`text-[10px] font-bold px-1.5 py-0.2 rounded border ${
                        act.status === 'Present' ? 'bg-emerald-100 text-emerald-800 border-emerald-300' : 'bg-rose-100 text-rose-800 border-rose-300'
                      }`}
                    >
                      {act.status}
                    </span>
          ...
          <div className="space-y-1.5">
            {recentExams.length > 0 ? (
              recentExams.map((exam) => (
                <div key={exam.eid} className="p-2.5 bg-amber-100/50 hover:bg-amber-100/80 border border-amber-200/90 rounded-xl flex items-center justify-between text-xs transition-colors shadow-2xs">
                  <div>
                    <span className="font-bold text-slate-900 text-[11px] block">{exam.subjectAndTopic}</span>
                    <span className="text-[10px] text-slate-600 font-mono">{exam.date}</span>
                  </div>
